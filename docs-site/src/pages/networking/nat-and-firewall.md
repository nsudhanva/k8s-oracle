...
3. **The "Trap": Firewall Rules**:
   Ubuntu 24.04 on OCI comes with `netfilter-persistent` and a default `REJECT` rule in the `FORWARD` chain. **Packets will reach the node but won't exit without these commands**:
   ```bash
