...
gemma_api_key = "secure-random-key" # API key for Gemma LLM authentication (unused while scaled to 0)

openclaw_gateway_token = "random-hex-string"  # openssl rand -hex 32
telegram_bot_token     = "your-botfather-token"
gemini_api_key         = "your-google-api-key" # Google Gemini API key for OpenClaw

google_places_api_key = "your-google-places-key" # Google Places API (optional)
discord_bot_token     = "your-discord-bot-token" # Discord bot token (optional)
gog_keyring_password  = "your-keyring-password"  # Google Workspace CLI keyring, legacy (optional)

alphavantage_api_key = "your-alphavantage-key"   # Alpha Vantage financial data API (optional)
bw_client_id         = "your-bitwarden-client-id" # Bitwarden CLI client ID (optional)
bw_client_secret     = "your-bitwarden-secret"    # Bitwarden CLI client secret (optional)
```
...
| `gemma_api_key` | API Key for Gemma LLM authentication (unused while scaled to 0) | Yes |
| `gemini_api_key` | Google Gemini API key for OpenClaw | Yes |
| `openclaw_gateway_token` | OpenClaw web UI auth token | Yes |
| `telegram_bot_token` | Telegram bot token from BotFather | Yes |
| `google_places_api_key` | Google Places API key (optional) | Yes |
| `discord_bot_token` | Discord bot token (optional) | Yes |
| `gog_keyring_password` | Google Workspace CLI keyring password, legacy (optional) | Yes |
| `alphavantage_api_key` | Alpha Vantage financial data API key (optional) | Yes |
| `bw_client_id` | Bitwarden CLI client ID (optional) | Yes |
| `bw_client_secret` | Bitwarden CLI client secret (optional) | Yes |
