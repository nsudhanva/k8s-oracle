...
description: Deploy an OKE Kubernetes cluster on Oracle Cloud Infrastructure (OCI) Free tier. Uses Terraform, ArgoCD GitOps, Envoy Gateway, automatic TLS with Let's Encrypt, and Cloudflare DNS.
template: splash
hero:
  tagline: Kubernetes on Oracle Cloud Always Free tier using managed control plane and GitOps.
  actions:
    - text: Get Started
...
<CardGrid>
  <Card title="OKE Basic Cluster" icon="setting">
  ...
  <Card title="OpenClaw AI Agent" icon="puzzle">
    Self-hosted AI agent with Telegram bot, Gemini API, and headless browser control. Managed declaratively via the OpenClaw operator.
  </Card>
</CardGrid>
...
| **LLM** | llama.cpp + Gemma 4 E2B | Self-hosted AI with OpenAI API |
