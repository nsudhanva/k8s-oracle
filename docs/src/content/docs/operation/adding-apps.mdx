---
title: Deploy Applications to OKE with ArgoCD GitOps
description: Step-by-step guide to deploy new applications on OKE using ArgoCD GitOps. Create Kubernetes manifests, HTTPRoutes, and TLS certificates with automatic sync.
---
...
3. **Add to applications.yaml**
   ...
   spec:
     project: default
     source:
       repoURL: https://github.com/nsudhanva/k8s-oracle.git
       targetRevision: HEAD
...
The `regcred` secret is managed by External Secrets Operator and automatically synced from OCI Vault.
