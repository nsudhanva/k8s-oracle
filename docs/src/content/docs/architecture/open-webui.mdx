...
**Cause:** OCI Identity Domain is using `response_mode=form_post` which sends the callback via POST, but Open WebUI expects GET.

**Fix:** In OCI Console, configure the application to use `response_mode=query`:
1. Navigate to **Applications → open-webui → OAuth configuration**
2. Ensure the response mode is set to query (not form_post)
3. Or verify the redirect URL is exactly: `https://chat.k8s.sudhanva.me/oauth/oidc/callback`
