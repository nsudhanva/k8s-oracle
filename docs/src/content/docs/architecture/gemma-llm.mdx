...
<Aside type="caution" title="Currently Scaled to Zero">
Gemma is scaled to 0 replicas. OpenClaw now uses the Google Gemini API instead of local inference. The deployment is kept for potential future GPU-based local inference. To re-enable, set `replicas: 1` in `argocd/apps/gemma/deployment.yaml`.
</Aside>

This deployment runs **Gemma 4 E2B IT**, Google's small Gemma 4 instruction-tuned model, using [llama.cpp](https://github.com/ggml-org/llama.cpp) with a Q4_K_M GGUF for CPU inference on ARM64 nodes.
...
| `llama-server` | `ghcr.io/nsudhanva/llama-server:b8638` | LLM inference | 3-6 GB, 1.2-2 CPU |
...
flowchart LR
    subgraph Config["llama-server Configuration"]
        ...
        Context["-c 16384<br/>Context Length"]
...
| `-c` | `16384` | Deployment context cap (the model supports up to 128K; 16K needed for OpenClaw) |
| `-ngl` | `0` | GPU layers (0 = CPU only) |
| `--threads` | `2` | Parallel inference threads |
| `--batch-size` | `2048` | Batch size for prompt processing (larger batches reduce overhead for long prompts) |
| `-np` | `1` | Single parallel slot to give full 16K context to one request |
