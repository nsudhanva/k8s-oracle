...
The OCI Always Free tier provides ~9.4GB allocatable memory per node. vLLM v0.13 on ARM64 CPU with Gemma 3 1B requires careful resource tuning. If you experience OOM issues during inference, consider:
- Reducing `VLLM_CPU_KVCACHE_SPACE` to 2
- Using `--max-model-len 2048` instead of 4096
- Switching to Ollama for more memory-efficient inference on constrained hardware
:::
