...
This cluster runs **OpenClaw v2026.4.15**, an open-source AI agent platform that connects messaging channels (Telegram, Discord, etc.) to LLM providers. It uses the **Google Gemini API** for fast, cloud-based inference and a **custom Docker image** with Chromium and CLI tools baked in.
...
flowchart TB
    ...
    subgraph OKE["OKE Cluster"]
        ...
        subgraph OpenClawPod["OpenClaw Pod"]
            ...
            Browser[Chromium<br/>Xvfb Headful]
        end
...
| **Browser Control** | Chromium via Xvfb headful mode for web browsing and scraping |
| **Memory + Dreaming** | Persistent memory with background consolidation |
| **Session Isolation** | Per-sender sessions with 30min idle auto-reset |
...
**What it adds on top of `ghcr.io/openclaw/openclaw:2026.4.15`:**
...
| **Google Tools** | `gws` (Google Workspace CLI - Gmail, Calendar, Drive, Docs, Sheets, Tasks, Chat, etc.), `goplaces` (Google Maps/Places) |
| **Media** | `ffmpeg` (audio/video processing) |
| **Dev Tools** | `gh` (GitHub CLI), `jq`, `ripgrep`, `tmux`, `python3`, `uv` (Python package manager), Python 3.14 via uv |
| **Security** | `bw` (Bitwarden CLI) for password manager access |
...
- Debian `chromium` package installs the browser at `/usr/bin/chromium`; Playwright's Chromium is installed separately via the bundled CLI for CDP/navigate support
...
| CPU | 500m | 2000m |
| Storage | 2 GB PVC | - |
| Shared Memory | 1 GB tmpfs | - |

<Aside type="note">
The `/dev/shm` tmpfs mount (1Gi) is required for Chrome. Without it, Chrome crashes with shared memory errors in containers.
</Aside>
...
```json
{
  ...
  "browser": {
    ...
    "profiles": {
      "remote": {
        "cdpUrl": "http://chromium-local:9223",
        "color": "#FF4500"
      }
    }
  },
...
- **Context window** is 131072 (Gemini's native 128K context). Safeguard compaction keeps 24K tokens reserved.
- **`botToken`** is NOT in the ConfigMap. It's injected by the init container (see below).

### Invalid Config Keys (v2026.4.15)
...
| `DISCORD_BOT_TOKEN` | (from secret, optional) | Discord bot authentication |
| `GOOGLE_WORKSPACE_CLI_KEYRING_BACKEND` | `file` | gws credential storage backend |
| `ALPHAVANTAGE_API_KEY` | (from secret) | Alpha Vantage stock/financial data API |
| `BW_CLIENTID` | (from secret) | Bitwarden CLI client ID |
| `BW_CLIENTSECRET` | (from secret) | Bitwarden CLI client secret |
| `PATH` | (extended) | Includes uv-managed Python 3.14 bin directory |
...
1. **Copies config files** from ConfigMap to PVC (`openclaw.json`, `AGENTS.md`, `TOOLS.md`, `USER.md`)
...
v2026.4.15 does NOT support `tokenFile` for Telegram (the gateway can't read the file at runtime). The `botToken` field must be set directly in the JSON config. Since we can't put secrets in the ConfigMap (committed to Git), the init container reads from the K8s Secret mount and injects it.
</Aside>
...
| `google-places-api-key` | `google-places-api-key` | `api-key` | Google Places API |
| `discord-bot-token` | `discord-bot-token` | `token` | Discord bot (optional) |
| `github-pat` | `github-pat` | `github-pat` | GitHub CLI |
| `alphavantage-api-key` | `alphavantage-api-key` | `api-key` | Alpha Vantage financial data API |
| `bw-credentials` | `bw-credentials` | `client-id`, `client-secret` | Bitwarden CLI authentication |
...
OpenClaw v2026.4.15 launches its own Chrome process via Playwright CDP. It does NOT support connecting to external browsers.
...
**Architecture:**
1. The custom entrypoint starts Debian `chromium` via Xvfb (headful mode) on `127.0.0.1:9222`
...
- Chrome timeout patches in `cdp.helpers-CJUeV0SO.js` (as backup)
...
- **v2026.4.15 startup delay:** Telegram polling takes ~30-40s after gateway ready
...
- **`detectedBrowser: unknown`**: Chrome not found. Check `/usr/bin/chromium` exists. Rebuild the custom image.
...
| File | Purpose |
|------|---------|
| `argocd/apps/openclaw/deployment.yaml` | PVC + Deployment (main manifest) |
| `argocd/apps/openclaw/configmap.yaml` | ConfigMap for openclaw.json |
| `argocd/apps/openclaw/configmap-agents.yaml` | ConfigMap for AGENTS.md |
