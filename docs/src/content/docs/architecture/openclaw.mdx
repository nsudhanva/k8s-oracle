...
This cluster runs **OpenClaw v2026.4.9**, an open-source AI agent platform that connects messaging channels (Telegram, Discord, etc.) to LLM providers. It uses the **Google Gemini API** for fast, cloud-based inference and a **custom Docker image** with Chromium and CLI tools baked in.
...
v2026.4.9 does NOT support `tokenFile` for Telegram (the gateway can't read the file at runtime). The `botToken` field must be set directly in the JSON config. Since we can't put secrets in the ConfigMap (committed to Git), the init container reads from the K8s Secret mount and injects it.
</Aside>
...
OpenClaw v2026.4.9 launches its own Chrome process via Playwright CDP. It does NOT support connecting to external browsers.

**Requirements for browser to work:**

1. Chrome must be installed in the container at a standard path (`/usr/bin/chromium` or `/usr/bin/google-chrome`)
2. `browser.noSandbox: true` in config (containers don't have sandbox capabilities)
3. `/dev/shm` must be mounted as tmpfs with sufficient size (256Mi)
4. Stale Chrome `Singleton*` lock files must be cleaned on startup (init container handles this)
5. `NODE_OPTIONS=--dns-result-order=ipv4first` to fix IPv6 networking issues

**How it works:** When the agent needs to browse, it starts Chrome headless on port 18800, connects via CDP, navigates pages, takes screenshots, extracts text, clicks elements, and fills forms.
...
- **v2026.4.9 startup delay:** Telegram polling takes ~30-40s after gateway ready
