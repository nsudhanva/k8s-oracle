  ...
  "models": {
    "providers": {
      "gemma": {
        ...
        "models": [{ "id": "gemma-4-E2B-it", "name": "Gemma 4 E2B", "contextWindow": 16384, "maxTokens": 2048 }]
      }
    }
  },
  "agents": {
...
- **Gateway binds to loopback** (`127.0.0.1`) by default in v2026.3.1. Health probes use `exec` with `wget` to reach localhost.
- **Telegram token** is mounted as a file from a Kubernetes Secret, not set via env var.
...
- **Context window** must match the llama-server `-c` flag (16384). OpenClaw requires minimum 16000 tokens.
- **Agent timeout** is set to 900s (15 min). OpenClaw's built-in system prompt is ~12K tokens, and CPU-only prompt processing on ARM takes ~12 minutes for the first message. Subsequent messages use llama.cpp's prompt cache and are much faster.
